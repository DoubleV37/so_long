...
/*   Updated: 2022/12/02 10:45:51 by vviovi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_invalid_contentmap(t_vars *vars, size_t x, size_t *y)
{
	if ((*y == 0 || *y == vars->map.size_y - 1 || x == 0
		...
		return (1);
	else if (vars->map.map[*y][x] != 'A' && vars->map.map[*y][x] != '0'
		&& vars->map.map[*y][x] != '1' && vars->map.map[*y][x] != 'E'
		&& vars->map.map[*y][x] != 'P' && vars->map.map[*y][x] != 'C')
		return (1);
	else
		return (0);
}
